...
def rollout():
    ...
    num_envs = 1
    obs = torch.zeros((config.num_steps, num_envs) + envs.single_observation_space.shape).to(config.device)
    actions = torch.zeros((config.num_steps, num_envs) + envs.single_action_space.shape).to(config.device)
    logprobs = torch.zeros((config.num_steps, num_envs)).to(config.device)
    rewards = torch.zeros((config.num_steps, num_envs)).to(config.device)
    dones = torch.zeros((config.num_steps, num_envs)).to(config.device)
    values = torch.zeros((config.num_steps, num_envs)).to(config.device)
    avg_returns = []

    next_obs, _ = envs.reset(seed=0)
    next_obs = torch.Tensor(next_obs).to(config.device)
    next_done = torch.zeros(num_envs).to(config.device)
    ret = 0
    ...
    with torch.no_grad():
        ...
        for t in reversed(range(config.num_steps)):
            ...
            delta = rewards[t] + config.gae_gamma * nextvalues * nextnonterminal - values[t]
            advantages[t] = lastgaelam = delta + config.gae_gamma * config.gae_lambda * nextnonterminal * lastgaelam
    ...
    return b_obs, b_actions, b_logprobs, b_advantages, b_values, b_returns, avg_returns
"""
...
"""
# update de CleanRL recopiée et modifiée
def update(obs, actions, old_logp, adv, old_values, rets):
    ...
    for _ in range(config.num_epochs):
        ...
        for start in range(0, 1 * config.num_steps, config.batch_size):
            ...
            with torch.no_grad():
                ...
                clipfracs += [((ratio - 1.0).abs() > config.clip_ratio).float().mean().item()]
            ...
            pg_loss2 = -mb_advantages * torch.clamp(ratio, 1 - config.clip_ratio, 1 + config.clip_ratio)
            pg_loss = torch.max(pg_loss1, pg_loss2).mean()

            # Value loss
            b_values = b_values.view(-1)
            if config.clip_vf:
                v_loss_unclipped = (b_values - b_rets) ** 2
                v_clipped = b_old_values + torch.clamp(
                    b_values - b_old_values,
                    -config.clip_ratio,
                    config.clip_ratio,
                )
                v_loss_clipped = (v_clipped - b_rets) ** 2
                v_loss_max = torch.max(v_loss_unclipped, v_loss_clipped)
                v_loss = 0.5 * v_loss_max.mean()
            else:
                v_loss = 0.5 * ((b_values - b_rets) ** 2).mean()
            ...
            optim.zero_grad()
            loss.backward()
            gn = nn.utils.clip_grad_norm_(agent.parameters(), config.max_grad_norm)
            optim.step()
...
def update(obs, actions, old_logp, adv, old_values, rets):
    ...
    for epoch in range(config.num_epochs):
        ...
        for start in range(0, config.num_steps, config.batch_size):
            ...
            _, newlogprob, entropy, newvalue = agent.get_action_value(obs[mb_inds], actions[mb_inds])
            logratio = newlogprob - old_logp[mb_inds]
            ratio = logratio.exp()
            ...
            if config.clip_vf:
                v_loss_unclipped = (newvalue - rets[mb_inds]) ** 2
                v_clipped = old_values[mb_inds] + torch.clamp(
                    newvalue - old_values[mb_inds],
                    -config.clip_ratio,
                    config.clip_ratio,
                )
                v_loss_clipped = (v_clipped - rets[mb_inds]) ** 2
                v_loss_max = torch.max(v_loss_unclipped, v_loss_clipped)
                v_loss = 0.5 * v_loss_max.mean()
            else:
                v_loss = 0.5 * ((newvalue - rets[mb_inds]) ** 2).mean()
